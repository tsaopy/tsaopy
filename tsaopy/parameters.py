...
class Fixed:
    """
    This class represents a fixed tsaopy parameter object used to set up the tsaopy model. It will
    ...
    Attributes:
    value(int or float): numerical value associated with the parameter. If parameter is Fixed it's
    the value assigned. If parameter is Fitting it's the value supplied to the initial values of
    the chain.
    ptype(str): ptype shand for parameter type. It's a string with the values x0, v0, a, b, c, or
...
class Fitting(Fixed):
    ...
    Attributes:
    value(int or float): estimation of the unknown variable that will be supplied to the initial
    values of the chain.
    ptype(str): see Fixed parameter class documentation.
    index(int or int touple): see Fixed parameter class documentation.
    prior(callable): a normalized probability density function represented as a callable. This
    object must return a number when being called with a number as argument.
    """
